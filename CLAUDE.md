...
change; these files are the project's only record of _why_, not just _what_.
...
- You don't need to run `npm run build` during normal development — Vite's dev server hot-reloads.
  **Exception**: previewing via the `frase.test` Herd environment when the dev server is _not_
