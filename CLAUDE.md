...
- **Card generation variants**: `AI::getContentForCard` (bilingual, no context) and `AI::getContentForCardWithContext` write `sentence`/`question`/`definition` in the target language and `translation` in the native language, steered by the learner's CEFR level (`levelInstruction`). `AI::getContentForCardNative` is the **monolingual** variant used when the save destination *is* the user's native language (`AjaxController@index` branches on `$language->id === $user->native_language_id`): every field is written in the native language, there is **no `translation`** (its schema omits it; the card stores `translation = ''` since the column is `NOT NULL`), and no CEFR steering is applied. It still expands single words into collocations and keeps the `[phrase]` bracket + question rules. It takes an optional `$context` to cover both the plain and context-supplied cases in one method.
