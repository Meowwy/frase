...
class GenerateEmbeddingJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            // NOTE: Automatic embedding-based linking is paused (work in progress).
            // Card links (the `synonyms` table) are currently created only manually by
            // the user from the card detail page. The similarity code below is kept so
            // the automatic system can be re-enabled once it is reliable. Re-enabling it
            // will start populating the same "Linked cards" table shown to the user.
            //
            ...
            Log::info('Embedding generated for card '.$this->card->id);
        } catch (\Exception $e) {
