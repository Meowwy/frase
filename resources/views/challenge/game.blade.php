@props(['maxTurns' => 10])
<x-html-layout>
    <div class="max-w-2xl mx-auto relative">
        <a href="/" class="inline-flex items-center gap-1 text-white/70 hover:text-white transition-colors mb-4">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
            <span>Home</span>
        </a>

        <div class="mb-3 flex items-baseline justify-between">
            <h1 class="text-lg font-bold">Conversation game</h1>
            <span class="text-sm text-white/60">Turn <span id="turnCounter" class="font-bold text-white">1</span> / {{ $maxTurns }}</span>
        </div>

        {{-- Scene setup (native language), shown once the opening loads. --}}
        <div id="sceneBanner" class="hidden mb-4 rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm">
            <span class="text-white/40">Scene:</span>
            <span id="sceneText" class="text-white/85"></span>
        </div>

        {{-- Loading state while the AI sets up the opening. --}}
        <div id="openingLoading" class="flex flex-col items-center text-center py-12">
            <svg class="animate-spin h-12 w-12 text-blue-500 mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p class="text-white/60 text-sm">Setting the scene…</p>
        </div>

        {{-- Message thread: assistant lines, task boxes, learner replies and their verdicts. --}}
        <div id="thread" class="space-y-3 mb-4"></div>

        {{-- Composer --}}
        <div id="composer" class="flex gap-2 items-end">
            <textarea id="chatInput" rows="1" autocomplete="off" placeholder="Write your reply in the target language…"
                      class="flex-1 resize-none max-h-40 overflow-y-auto rounded-lg border border-white/10 bg-[#111] px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-white/30"></textarea>
            <button id="sendBtn" type="button"
                    class="rounded-lg bg-[#ff6f61] hover:bg-[#d7574d] px-5 py-2 text-sm font-bold text-white transition-colors disabled:opacity-40">
                Send
            </button>
        </div>

        {{-- Feedback (revealed once the game ends). --}}
        <div id="recapPanel" class="hidden mt-8 rounded-xl border border-white/10 bg-white/5 p-5">
            <h3 id="recapHeading" class="text-lg font-semibold mb-3"></h3>
            <p id="recapMistake" class="hidden text-sm text-white/85 mb-3"></p>
            <p id="recapScore" class="text-sm text-white/60"></p>
            <div class="mt-5">
                <a href="/conversation" class="inline-flex items-center gap-2 rounded-lg bg-[#ff6f61] hover:bg-[#d7574d] px-5 py-2 text-sm font-bold text-white transition-colors">Play again</a>
                <a href="/" class="inline-flex items-center gap-2 rounded-lg border border-white/15 bg-white/5 hover:bg-white/10 px-5 py-2 text-sm font-semibold text-white transition-colors ml-2">Home</a>
            </div>
        </div>
    </div>

    <script>
        $(document).ready(function () {
            const csrf = '{{ csrf_token() }}';
            const maxTurns = {{ $maxTurns }};

            const $thread = $('#thread');
            const $input = $('#chatInput');
            const $send = $('#sendBtn');
            const $turnCounter = $('#turnCounter');

            let ended = false;   // game over (win or loss)
            let busy = false;    // a request is in flight

            function escapeHtml(s) {
                return $('<div>').text(s == null ? '' : s).html();
            }

            function spinnerSvg(sizeClasses) {
                return '<svg class="animate-spin ' + sizeClasses + '" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">' +
                    '<circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>' +
                    '<path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>' +
                    '</svg>';
            }

            // Assistant line (target language) — left bubble.
            function appendAssistant(text) {
                const row = $('<div>').addClass('flex justify-start');
                row.append($('<div>').addClass('max-w-[90%] rounded-2xl bg-white/10 px-4 py-2 text-sm text-white').html(escapeHtml(text)));
                $thread.append(row);
                row[0].scrollIntoView({ behavior: 'smooth', block: 'end' });
            }

            // The task the learner must accomplish next (native language) — highlighted box.
            function appendTask(text) {
                const box = $('<div>').addClass('rounded-xl border border-blue-400/30 bg-blue-400/5 px-4 py-3');
                box.append($('<div>').addClass('text-xs font-semibold uppercase tracking-wide text-blue-300 mb-1').text('🎯 Your task'));
                box.append($('<p>').addClass('text-sm text-white/90').html(escapeHtml(text)));
                $thread.append(box);
                box[0].scrollIntoView({ behavior: 'smooth', block: 'end' });
            }

            // Learner reply — right bubble; returns the verdict cell placed below it.
            function appendUser(text) {
                const wrap = $('<div>').addClass('flex flex-col items-end gap-1');
                wrap.append($('<div>').addClass('max-w-[90%] rounded-2xl bg-[#ff6f61] px-4 py-2 text-sm text-white').html(escapeHtml(text)));
                const verdict = $('<div>').addClass('flex items-center gap-2 text-xs text-white/50').html(spinnerSvg('h-4 w-4'));
                wrap.append(verdict);
                $thread.append(wrap);
                wrap[0].scrollIntoView({ behavior: 'smooth', block: 'end' });
                return verdict;
            }

            // Typing bubble while the AI judges the turn.
            function showTyping() {
                const row = $('<div id="typingRow">').addClass('flex justify-start');
                row.append($('<div>').addClass('rounded-2xl bg-white/10 px-4 py-2 text-white/60 flex items-center').html(spinnerSvg('h-5 w-5')));
                $thread.append(row);
                row[0].scrollIntoView({ behavior: 'smooth', block: 'end' });
            }

            function hideTyping() {
                $('#typingRow').remove();
            }

            function chip(ok, okText, badText) {
                const cls = ok ? 'text-green-400' : 'text-red-400';
                const mark = ok ? '✓' : '✗';
                return '<span class="inline-flex items-center gap-1 ' + cls + '"><span>' + mark + '</span><span>' + (ok ? okText : badText) + '</span></span>';
            }

            function renderVerdict(cell, taskDone, hasError) {
                cell.removeClass('flex items-center').addClass('flex items-center')
                    .html(
                        chip(taskDone, 'Task done', 'Task not completed') +
                        chip(!hasError, 'No errors', 'Grammar mistake')
                    );
            }

            function lockComposer() {
                $input.prop('disabled', true);
                $send.prop('disabled', true);
            }

            function autoGrow() {
                const el = $input[0];
                el.style.height = 'auto';
                el.style.height = el.scrollHeight + 'px';
            }

            function showFeedback(data) {
                const $heading = $('#recapHeading');
                const $mistake = $('#recapMistake');

                if (data.completed) {
                    $heading.text('🎉 You cleared all ' + maxTurns + ' turns!').addClass('text-green-400');
                    $mistake.addClass('hidden').empty();
                } else {
                    $heading.text('Game over').addClass('text-orange-300');
                    if (data.mistake) {
                        $mistake.text(data.mistake).removeClass('hidden');
                    } else {
                        $mistake.addClass('hidden').empty();
                    }
                }

                const n = data.turns_survived || 0;
                $('#recapScore').text('You lasted ' + n + ' ' + (n === 1 ? 'turn' : 'turns') + '.');
                $('#recapPanel').removeClass('hidden')[0].scrollIntoView({ behavior: 'smooth', block: 'start' });
            }

            function sendMessage() {
                const text = $input.val().trim();
                if (busy || ended || text === '') { return; }

                busy = true;
                $send.prop('disabled', true);
                const cell = appendUser(text);
                $input.val('');
                autoGrow();
                showTyping();

                $.post('/conversation/game/message', { message: text, _token: csrf }, function (data) {
                    hideTyping();
                    renderVerdict(cell, data.task_completed, data.has_error);

                    if (data.ended) {
                        ended = true;
                        lockComposer();
                        // A win still gets the AI's closing in-character line first.
                        if (data.completed && data.reply) { appendAssistant(data.reply); }
                        showFeedback(data);
                        return;
                    }

                    // Passed — advance to the next task.
                    $turnCounter.text((data.turns_survived || 0) + 1);
                    appendAssistant(data.reply);
                    appendTask(data.suggestion);
                }).fail(function (xhr) {
                    hideTyping();
                    cell.removeClass('flex items-center').html('<span class="text-red-400/80">Could not check this reply.</span>');
                    const msg = (xhr.responseJSON && xhr.responseJSON.message) || 'Something went wrong. Please try again.';
                    if (window.toastr) { toastr.error(msg); }
                }).always(function () {
                    busy = false;
                    if (!ended) {
                        $send.prop('disabled', false);
                        $input.focus();
                    }
                });
            }

            // Fetch the scene + opening line + first task after the page is visible.
            function loadOpening() {
                lockComposer();

                $.post('/conversation/game/opening', { _token: csrf }, function (data) {
                    $('#openingLoading').remove();
                    if (data.scene) {
                        $('#sceneText').text(data.scene);
                        $('#sceneBanner').removeClass('hidden');
                    }
                    appendAssistant(data.reply);
                    appendTask(data.suggestion);
                    $input.prop('disabled', false);
                    $send.prop('disabled', false);
                    $input.focus();
                }).fail(function (xhr) {
                    $('#openingLoading').html(
                        '<p class="text-red-400/80 text-sm">Could not start the game. ' +
                        '<a href="/conversation" class="underline hover:text-red-300">Try again</a>.</p>'
                    );
                    const msg = (xhr.responseJSON && xhr.responseJSON.message) || 'Could not start the game.';
                    if (window.toastr) { toastr.error(msg); }
                });
            }

            $send.on('click', sendMessage);
            $input.on('input', autoGrow);
            $input.on('keydown', function (e) {
                if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); }
            });

            loadOpening();
        });
    </script>
</x-html-layout>
