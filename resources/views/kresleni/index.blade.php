...
<button onclick="generateItem()">Vygenerovat výzvu</button>
<div id="result"></div>
<div class="controls">
<button onclick="handleGuess(true)">Uhodnuto ✓</button>
<button onclick="handleGuess(false)">Neuhodnuto ✗</button>
</div>
...
<script>
const categories = {
zvirata: ["lev", "slon", "medvěd", "žirafa", "tygr", "panda", "krokodýl", "velryba", "ptakopysk", "oktopus", "liška", "vlk", "ještěr", "sov", "žába", "korál", "medúza", "lenochod", "surikata", "pavouk", "mlok", "klokan", "nosorožec", "tapír", "luskoun", "kajman", "puma", "rys", "bizon", "lemur", "hroch", "zebra", "antilopa", "kobyla", "albatros", "kondor"],
jidlo: ["piza", "sushi", "hamburger", "taco", "ramen", "lasagne", "smoothie", "bageta", "cupcake", "sendvič", "borscht", "fondue", "gnocchi", "risotto", "sashimi", "burrito", "croissant", "donut", "falafel", "gelato", "guacamole", "karamel", "muffin", "nachos", "obložený chléb", "pancake", "quesadilla", "ratatouille", "strudel", "tiramisu", "vafle", "zavináč", "štrúdl", "česnečka", "řízek", "směs ovoce"],
...
<label><input type="radio" name="category" value="${cat}"> ${cat.charAt(0).toUpperCase() + cat.slice(1)}</label>
`).join('');
...
document.getElementById('current-player').textContent = `Na řadě: ${players[currentPlayerIndex]}`;
}
...
function handleGuess(success) {
if(document.getElementById('result').textContent === "") return;

if(success) {
const currentScore = parseInt(document.querySelectorAll('.player-score')[currentPlayerIndex].querySelector('.points').textContent);
document.querySelectorAll('.player-score')[currentPlayerIndex].querySelector('.points').textContent = currentScore + 1;
}
